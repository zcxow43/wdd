...
One `spread_default` row exists per brand from the moment that brand is seeded (`specs/dba/spread-default.md`) — there is no create/delete endpoint. `SpreadDefaultAuditHandler` plugs `spread_default` updates into the audit module as `entityType = "SPREAD_DEFAULT"`.
