...
- Status: DONE
- Re-executed from scratch after the 2026-08-03 teardown, against the current codebase snapshot, whose base package is `com.wdd.backend` (not `pl.piomin.services.backend` referenced by the pre-teardown Execution Result history above — that was a different snapshot). Built the base CRUD and the "Delta: deletion requires every brand's pair to be inactive first" section together in one pass, per instructions.
- Files changed:
  - `develop/backend/src/main/java/com/wdd/backend/model/CurrencyPairDefinition.java` (new — entity, 1:1 with `currency_pair_definition`, plus `baseCurrencyCode`/`quoteCurrencyCode` enrichment fields populated only by the joined read queries)
  - `develop/backend/src/main/java/com/wdd/backend/dto/CurrencyPairDefinitionResponse.java` (new)
  - `develop/backend/src/main/java/com/wdd/backend/dto/CurrencyPairDefinitionCreateRequest.java` (new — `baseCurrencyId`/`quoteCurrencyId`/`forwardPrecision`/`reversePrecision` all `@NotNull`, precision fields `@Min(0)`/`@Max(8)`)
  - `develop/backend/src/main/java/com/wdd/backend/dto/CurrencyPairDefinitionUpdateRequest.java` (new — `forwardPrecision`/`reversePrecision` only, no currency fields at all)
  - `develop/backend/src/main/java/com/wdd/backend/mapper/CurrencyPairDefinitionMapper.java` (new — `findAll` (optional base/quote filters), `findById` (enriched via joins to `currency` twice), `findByEitherDirection` (direction-independent pre-check mirroring the DB's `pair_key_low`/`pair_key_high` unique index), `insert`, `update`, `deleteById`)
  - `develop/backend/src/main/resources/mapper/CurrencyPairDefinitionMapper.xml` (new — MyBatis SQL mapper, reusing the enriched-join pattern from `CurrencyPairMapper.xml`)
  - `develop/backend/src/main/java/com/wdd/backend/service/CurrencyPairDefinitionService.java` (new — `list`, `getById`, `create` (`@Transactional`: validates currencies exist and differ, pre-checks either-direction uniqueness via `findByEitherDirection`, inserts the definition, then fans out to every brand via `brandMapper.findAll(null)`, skipping any brand with an existing live `(brand, base, quote)` `currency_pair` row via `currencyPairMapper.findByBrandBaseQuote(brandId, baseCurrencyId, quoteCurrencyId, null)`, calling the existing unmodified `CurrencyPairService.create(...)` as a plain method for the rest with `rateType=AUTO`/`rate=null`/`active=true`), `update` (precision only), `delete` (loads the definition, calls `currencyPairMapper.findActiveByBaseQuote(baseCurrencyId, quoteCurrencyId)`, throws `CurrencyPairDefinitionInUseException` with the active rows' `brandCode`s if any are found, otherwise deletes only the `currency_pair_definition` row — built in from the start, per instructions, rather than as a separate delta pass)
  - `develop/backend/src/main/java/com/wdd/backend/controller/CurrencyPairDefinitionController.java` (new — `GET /api/currency-pair-definitions` (optional `baseCurrencyId`/`quoteCurrencyId`), `GET /{id}`, `POST` → `201`, `PUT /{id}` → `200`, `DELETE /{id}` → `204`; all mutations apply immediately, no audit-approval submission)
  - `develop/backend/src/main/java/com/wdd/backend/exception/CurrencyPairDefinitionNotFoundException.java` (new)
  - `develop/backend/src/main/java/com/wdd/backend/exception/CurrencyPairDefinitionExistsException.java` (new)
  - `develop/backend/src/main/java/com/wdd/backend/exception/CurrencyPairDefinitionInUseException.java` (new — carries `baseCurrencyId`, `quoteCurrencyId`, `activeBrandCodes`)
  - `develop/backend/src/main/java/com/wdd/backend/exception/GlobalExceptionHandler.java` (edited — added handlers for `CurrencyPairDefinitionNotFoundException` → `404`, `CurrencyPairDefinitionExistsException` → `409`, and `CurrencyPairDefinitionInUseException` → `409` with the `error`/`baseCurrencyId`/`quoteCurrencyId`/`activeBrandCodes` body; inserted alongside the existing handlers without touching any pre-existing one; reused `CurrencyNotFoundException` → `404` and `InvalidCurrencyPairException` → `400` handlers as-is)
  - `develop/backend/src/main/java/com/wdd/backend/mapper/CurrencyPairMapper.java` (edited — added `findActiveByBaseQuote(Long baseCurrencyId, Long quoteCurrencyId)`, purely additive, no change to any existing method signature/behavior)
  - `develop/backend/src/main/resources/mapper/CurrencyPairMapper.xml` (edited — added the `findActiveByBaseQuote` `<select>`, reusing the existing `enrichedSelect` `<sql>` fragment also used by `findAll`/`findById`; filters `WHERE cp.base_currency_id = ... AND cp.quote_currency_id = ... AND cp.active = TRUE`; no existing `<select>`/`<insert>`/`<update>`/`<delete>` touched)
  - `develop/backend/src/test/resources/schema.sql` (edited — added an H2-compatible `currency_pair_definition` table, intentionally without the production unique constraint on `(pair_key_low, pair_key_high)` since H2's generated-column/unique-index combo isn't replicated here — the service-layer `findByEitherDirection` pre-check is what the tests exercise, consistent with how `currency_pair` in this same test schema already omits FKs/DB-level guards that are production-only defense-in-depth)
  - `develop/backend/src/test/java/com/wdd/backend/service/CurrencyPairDefinitionServiceTest.java` (new — 19 unit tests, Mockito, covering list/getById, all `create` validation branches (currency not found ×2, base==quote, exact-direction exists, reverse-direction exists), the fan-out behavior including "skip brand with existing row" and a fan-out-failure-prevents-persistence case, `update` (success + not-found), and the delete guard (not-found, blocked with one active brand asserting exact `activeBrandCodes`, blocked with multiple active brands asserting the full ordered list, allowed once all inactive, allowed with zero rows))
  - `develop/backend/src/test/java/com/wdd/backend/controller/CurrencyPairDefinitionControllerTest.java` (new — 18 MockMvc integration tests against H2, covering create/fan-out-to-all-seeded-brands/skip-existing-brand-row/409-reverse-direction/409-duplicate-exact-direction/400-base-equals-quote/404-currency-missing/400-precision-out-of-range/list-filters-by-base-and-quote/getById-404/update-success/update-400/update-404/delete-success-leaves-currency-pairs-untouched/delete-404/delete-409-with-activeBrandCodes-when-any-brand-still-active/delete-succeeds-once-all-inactive/delete-succeeds-with-zero-rows/reverse-direction-succeeds-after-original-deleted)
  - `develop/backend/pom.xml` (edited — version bumped `0.0.3` → `0.0.4`, description updated to mention `currency-pair-definition`)
- Notes:
  - Confirmed via reading `CurrencyPairService.java`/`CurrencyPairController.java` in this snapshot that `create(CurrencyPairCreateRequest)` is already a plain, non-audited `@Transactional` public method and that `CurrencyPairController` has no `POST` route at all (a `currency_pair` row can only ever come into existence via this feature's fan-out, per this snapshot's own `CurrencyPairController`/`CurrencyPairService`/`CurrencyPairAuditHandler` Javadoc). `CurrencyPairDefinitionService.create` calls `CurrencyPairService.create` directly as a plain method, exactly as the spec requires, with zero changes to `CurrencyPairService`, `CurrencyPairValidator`, `CurrencyPairController`, or `CurrencyPairAuditHandler`.
  - The DB-level "no reverse pair" guard (unique index on generated `pair_key_low`/`pair_key_high` columns) already exists in the live MySQL `wdd` database per `specs/dba/currency-pair-definition.md`'s `V009` migration (confirmed present via `SHOW TABLES`/manual smoke test below); this spec's `CurrencyPairDefinitionService.create` implements the friendlier application-level pre-check (`findByEitherDirection`) as the primary error path, with the DB constraint remaining the backstop in production. The H2 test schema does not replicate the generated-column unique index, so tests validate the service-layer pre-check specifically.
  - Built the base CRUD and the deletion-guard delta together in a single implementation pass (not as two increments), per instructions — `CurrencyPairDefinitionService.delete`, `CurrencyPairMapper.findActiveByBaseQuote`, and `CurrencyPairDefinitionInUseException` all landed in this same increment, and every Delta Acceptance Criteria checkbox is checked alongside the base ones.
  - Ran `mvn -f develop/backend/pom.xml test` — **BUILD SUCCESS, 187 tests total, 0 failures/errors**, including the pre-existing `CurrencyPairControllerTest` (24), `CurrencyPairServiceTest` (26), and `CurrencyPairAuditHandlerTest` (18) suites passing completely unmodified (no edits made to any of those three files or to `CurrencyPairService`/`CurrencyPairValidator`/`CurrencyPairController`), confirming zero regression. New tests: 19 `CurrencyPairDefinitionServiceTest` (unit) + 18 `CurrencyPairDefinitionControllerTest` (integration) = 37 new tests.
  - Additionally ran a full live smoke test against the real MySQL `wdd` database (`mvn spring-boot:run` on port 8080, per `application.yml`, with the live-server port free-checked via `lsof` before starting and the server cleanly stopped afterward): created a (GBP, CAD) definition, confirmed a `currency_pair` row (`AUTO`/`rate=null`/`active=true`) was fanned out to all 7 live-seeded brands, confirmed the reverse direction (CAD, GBP) returned `409`, confirmed `PUT` updated precision (and rejected out-of-range with `400`), confirmed `DELETE` returned `409` with `activeBrandCodes` listing all 7 brand codes while active rows existed, then confirmed `DELETE` succeeded (`204`) once every row was deactivated and that the 7 provisioned `currency_pair` rows remained untouched afterward. All smoke-test data was cleaned up from the live database afterward (deleted the 7 test `currency_pair` rows; the test definition itself was already removed by the successful `DELETE`), leaving `wdd` in its prior state.
  - No migration file was created or modified, per instructions — `V009__create_currency_pair_definition_table.sql` was already applied to the live MySQL database by a prior DBA pipeline stage; confirmed present via `SHOW TABLES` before starting this increment.
  - `docker/launch.json` already had a correct `backend` entry (`mvn -f develop/backend/pom.xml spring-boot:run`, port `8080` matching `application.yml`) and `.claude/launch.json` was already a valid symlink to it — neither required any change.
