...
### Increment 7 — 2026-08-04 (Rebuild after teardown: V003 + V004 only)
- Status: DONE (V003/V004 portion only — `V011` intentionally deferred pending `V005` (`audit_request`) and `V009` (`currency_pair_definition`), neither of which exists yet in this fresh rebuild; front-matter `status` left as `pending` since V011 work remains)
- Files changed: none on disk — per current convention (see Increment 4 above), migration SQL lives only in this spec's `## Migration SQL` sections and is applied directly against the live database via the `mysql` CLI; no standalone `.sql` file was written anywhere.
- Notes:
  - Pre-flight: read `env.md` (Engine MySQL 8.0.36, Host 127.0.0.1:3306, DB `wdd`, user `app`, password `1234`); connectivity confirmed via `mysql -h 127.0.0.1 -P 3306 -u app -p1234 -e "SELECT 1;"`; confirmed database `wdd` already exists (`SHOW DATABASES LIKE 'wdd'`) — no creation needed.
  - Confirmed pre-existing state before running: `SHOW TABLES` → only `brand` and `currency` present; `DESCRIBE currency` → no `active` column (confirms `V010` already applied); `brand` = 7 rows, `currency` = 10 rows; `currency_pair` did not exist yet, confirming this run is correctly `V003`.
  - Applied `V003` (create `currency_pair` table + 4 seed rows: AU/USD/TWD, AU/EUR/TWD, MONETA/USD/JPY, VT/USD/EUR) directly against the live `wdd` database via the `mysql` CLI — succeeded on first attempt with no errors.
  - Applied `V004` (make `rate` nullable, clear existing `AUTO` rows to `NULL`, drop `ck_currency_pair_rate_positive` and add `ck_currency_pair_rate_valid`, insert 10 more seed rows covering `PUG`/`STAR`/`UM`/`VJP` plus extra rows for `AU`/`MONETA`) directly against the live database — succeeded on first attempt with no errors.
  - Verification performed against the live database (all V003/V004 acceptance criteria confirmed empirically):
    - `DESCRIBE currency_pair` → all 9 columns present with correct types; `rate` is `decimal(18,8)`, `Null: YES`, `DEFAULT NULL`.
    - `SHOW CREATE TABLE currency_pair` → confirms PK on `id`; UNIQUE `uk_currency_pair_brand_base_quote` on (`brand_id`,`base_currency_id`,`quote_currency_id`); FKs `fk_currency_pair_brand`/`fk_currency_pair_base`/`fk_currency_pair_quote` all `ON DELETE RESTRICT ON UPDATE RESTRICT`; CHECK constraints `ck_currency_pair_distinct`, `ck_currency_pair_rate_type`, and `ck_currency_pair_rate_valid` present — `ck_currency_pair_rate_positive` confirmed absent.
    - CHECK constraint rejection tests, all correctly rejected with `ERROR 3819`: `rate_type='MANUAL'` with `rate=NULL`, `rate=0`, and `rate=-5` (all three via `ck_currency_pair_rate_valid`); `rate_type='AUTO'` with `rate=99.99` (`ck_currency_pair_rate_valid`); `base_currency_id = quote_currency_id` (`ck_currency_pair_distinct`); `rate_type='BOGUS'` (`ck_currency_pair_rate_type`).
    - Unique constraint test: duplicate insert of existing (`brand_id=1,base=2,quote=1`) → `ERROR 1062 Duplicate entry '1-2-1'`.
    - FK insert test: `brand_id=999` (non-existent) → `ERROR 1452` on `fk_currency_pair_brand`.
    - FK delete-RESTRICT tests: `DELETE FROM brand WHERE id=1` (AU, referenced) → `ERROR 1451` on `fk_currency_pair_brand`; `DELETE FROM currency WHERE id=2` (USD, referenced) → `ERROR 1451` on `fk_currency_pair_base`; both correctly rejected, neither delete applied.
    - `updated_at` auto-update test: changed `rate` on row id=1 from 32.5 to 33.0, confirmed `updated_at` advanced while `created_at` stayed fixed, then restored the original value 32.5 (confirmed `updated_at` advanced again, since it was a real value change).
    - Final data: `SELECT COUNT(*) FROM currency_pair` = 14 (4 from V003 + 10 from V004). Brand coverage: all 7 brands have at least one pair — `AU` (3 MANUAL, 0 AUTO), `MONETA` (0 MANUAL, 2 AUTO), `PUG` (1 MANUAL, 1 AUTO), `STAR` (1 MANUAL, 1 AUTO), `UM` (1 MANUAL, 1 AUTO), `VJP` (1 MANUAL, 1 AUTO), `VT` (0 MANUAL, 1 AUTO) — matching the exact seed values in the spec's `V003`/`V004` SQL blocks. All AUTO rows have `rate = NULL`; all MANUAL rows have a positive `rate`. `brand` (7 rows) and `currency` (10 rows) unchanged throughout.
    - `currency_pair` `AUTO_INCREMENT` shows gaps (landed at 25 after the deliberate constraint-violation test inserts, which consumed ids but were rejected) — standard MySQL auto-increment reservation behavior, not a sign of duplicate/failed real inserts; the 14 actual rows (ids 1-4, 8-17) are exactly as expected with no duplicates or gaps in real data.
  - `V011` (data-reset of `audit_request`/`currency_pair_definition`) intentionally **not executed** this pass — its prerequisite tables (`audit_request` = `V005`, `currency_pair_definition` = `V009`) do not exist yet in this fresh rebuild (`SHOW TABLES` confirmed only `brand`, `currency`, `currency_pair` exist after this increment). Its Acceptance Criteria items remain unchecked. Front-matter `status` left as `pending` pending that follow-up dispatch, matching this same spec's prior precedent (Increment 5/6).

### Increment 8 — 2026-08-04 (Rebuild after teardown: V011 — completes the rebuild)
- Status: DONE (V011 portion — completes the rebuild; front-matter `status` now `done`)
- Files changed: none on disk — per current convention (Increment 4 above), migration SQL lives only in this spec's `## Migration SQL` section and was applied directly against the live database via the `mysql` CLI; no standalone `.sql` file was written anywhere.
- Notes:
  - Pre-flight: read `env.md` (Engine MySQL 8.0.36, Host 127.0.0.1:3306, DB `wdd`, user `app`, password `1234`); connectivity confirmed via `mysql -h 127.0.0.1 -P 3306 -u app -p1234 -e "SELECT 1;"`; confirmed database `wdd` already exists (`SHOW DATABASES LIKE 'wdd'`) — no creation needed.
  - Confirmed pre-existing state before running: `SHOW TABLES` → `audit_request`, `brand`, `currency`, `currency_pair`, `currency_pair_definition`, `spread_default`, `spread_group`, `spread_group_member` all present (this rebuild's `V005`–`V010` had already been applied in earlier passes this session, per the task context). Row counts before: `currency_pair` = 14 (the `V003`/`V004` orphaned seed rows applied in Increment 7), `currency_pair_definition` = 0, `audit_request` total = 0 (so `entity_type='CURRENCY_PAIR'` count was already 0), `spread_group_member` = 0, `brand` = 7, `currency` = 10, `spread_default` = 7, `spread_group` = 0.
  - Captured `SHOW CREATE TABLE` for all 8 tables (`currency_pair`, `currency_pair_definition`, `audit_request`, `spread_group_member`, `brand`, `currency`, `spread_default`, `spread_group`) before running the migration.
  - Applied the `V011` SQL (`DELETE FROM audit_request WHERE entity_type='CURRENCY_PAIR'; DELETE FROM currency_pair; DELETE FROM currency_pair_definition;`) directly against the live `wdd` database via the `mysql` CLI — no errors on first attempt.
  - Row counts after: `currency_pair` 14 → 0; `currency_pair_definition` 0 → 0; `audit_request` (`entity_type='CURRENCY_PAIR'`) 0 → 0; `audit_request` total 0 → 0; `spread_group_member` 0 → 0 (unchanged — nothing referenced a deleted pair, so no cascade fired); `brand` 7 → 7, `currency` 10 → 10, `spread_default` 7 → 7, `spread_group` 0 → 0 (all unchanged).
  - Re-captured `SHOW CREATE TABLE` for all 8 tables after the migration and diffed against the before-capture: byte-for-byte identical (`diff` produced no output) — confirms no column/index/constraint was changed by this purely-DML migration; `currency_pair`'s `AUTO_INCREMENT=25` counter is also unchanged since plain `DELETE` (not `TRUNCATE`) does not reset it.
  - Verified the DB-layer intent of the "no stale `409`" acceptance criterion directly with SQL: inside a transaction, inserted 1 `currency_pair_definition` row (base=1, quote=2, forward_precision=4, reverse_precision=4) followed by a fan-out `INSERT INTO currency_pair ... SELECT b.id, 1, 2, NULL, 'AUTO', 1 FROM brand b`, confirmed 1 definition row and 7 fanned-out pair rows (one per brand) inserted with no unique-constraint or check-constraint violations, then `ROLLBACK`ed, leaving both tables back at 0 rows.
  - Migration is not reversible (data deletion); rollback would require restoring from a backup taken before this ran, per the in-file comment. This was not needed — migration succeeded cleanly.
  - This completes the rebuild of this spec: `V003`, `V004` (Increment 7), and `V011` (this increment) are all now applied and verified against the live database this session. All remaining V011-related Acceptance Criteria items checked off above. Front-matter `status` set to `done`.
