...
### Increment 5 — 2026-08-04
- Status: DONE
- Files changed: none (no standalone `.sql` files are written per current convention; SQL lives only in this spec's `## Migration SQL` sections and was applied directly to the live database)
- Notes: Rebuilt from scratch after the 2026-08-04 teardown, per this file's instructions to execute the whole remaining spec in one pass (`V001` base creation immediately followed by the `V010` delta — V010 has no real functional dependency on any other table; the "V009 must come first" note is purely about migration-numbering sequence). Ran DBA pre-flight: `env.md` validated (Engine MySQL 8.0.36, Host 127.0.0.1:3306, Database wdd, User app, Password present); `mysql -h 127.0.0.1 -P 3306 -u app -p1234 -e "SELECT 1;"` succeeded; `SHOW DATABASES LIKE 'wdd'` confirmed the database already existed (no CREATE DATABASE needed). Confirmed `currency` did not yet exist (`SHOW TABLES LIKE 'currency'` returned empty), consistent with a fresh, fully torn-down database. Applied `V001` (CREATE TABLE `currency` with id/code/name/name_zh/symbol/decimal_places/active/created_at/updated_at, PK on id, UNIQUE key `uk_currency_code` on code) directly via `mysql` CLI, then inserted the 10 seed rows (TWD, USD, EUR, JPY, GBP, CNY, HKD, SGD, AUD, CAD). Verified via `DESCRIBE currency`, `SELECT COUNT(*)`, and `SHOW INDEX FROM currency` — table, PRIMARY/uk_currency_code indexes, and all 10 rows present with `active` defaulting to 1 and `decimal_places` defaulting to 2. Immediately applied `V010` (`ALTER TABLE currency DROP COLUMN active;`) directly via `mysql` CLI. Verified post-delta state: `DESCRIBE currency` confirms `active` is gone (remaining columns: id, code, name, name_zh, symbol, decimal_places, created_at, updated_at); `SELECT * FROM currency` returns all 10 seed rows with every other column/value unchanged; `SHOW TABLES` shows only `currency` exists in the freshly rebuilt database (no `brand`/`currency_pair`/`currency_pair_definition`/`spread_*` tables present yet, so none could have been or were altered by this migration). All Acceptance Criteria (base list and the "Delta: drop the active column" list) checked off; frontmatter `status` updated to `done` as this completes the entire spec.
