...
- Notes:
  - Pre-flight passed: connected to MySQL 8.0.36 at `127.0.0.1:3306`, database `wdd`, user `app`, via `mysql` CLI `SELECT 1;`; database `wdd` already existed (no creation needed).
  - Confirmed pre-migration state: `SHOW TABLES` listed only `audit_request`, `brand`, `currency`, `currency_pair` — `spread_default` did not yet exist; `brand` had 7 rows.
  ...
  - Verified CHECK constraint: `UPDATE spread_default SET withdraw_spread = -5 WHERE brand_id = 1;` raised `ERROR 3819: Check constraint 'ck_spread_default_withdraw_nonneg' is violated`; row for `brand_id = 1` remains unchanged (`0.00000000`/`0.00000000`).
